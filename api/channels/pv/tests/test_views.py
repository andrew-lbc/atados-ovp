...
class MeetingViewSetTestCase(TestCase):
  ...
  def test_list_meeting_dates(self):
    ...
    response = self.client.get(reverse("meeting-list"), format="json", HTTP_X_OVP_CHANNEL="pv")
  ...
  def test_can_unappoint(self):
    pk = models.PVMeeting.objects.last().pk
    response = self.client.post(reverse("meeting-unappoint", [pk]), format="json", HTTP_X_OVP_CHANNEL="pv")
    self.assertEqual(response.status_code, 401)
